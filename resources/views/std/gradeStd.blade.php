...
@section('content')
    ...
    <div class="flex justify-center items-center ml-60 h-full bg-gray-200 p-8">
        <div class="w-5/6 h-5/6 bg-white rounded-xl shadow-xl">
            ...
            @if($regisCount == 0)
                <p class="ml-12 mt-5 text-xl text-kmutt-or">You didn't enroll any class.</p>
            @else
                @if(!empty($mssg))
                    <p class="ml-14 mt-5 text-xl text-kmutt-or">GPAX: {{ $mssg }}</p>
                @else
                    <p class="ml-14 mt-5 text-xl text-kmutt-or">GPAX: {{ $GPAX }}</p>
                @endif
